...
export default function NewIssuePage() {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    if (!res.ok) {
      setError(
        data.error ? JSON.stringify(data.error) : "Failed to create issue"
      );
      return;
    }
  ...
  return (
    <div className="max-w-lg">
      <h1 className="text-2xl font-semibold text-slate-50">New Issue</h1>
      <form onSubmit={handleSubmit} className="mt-6 space-y-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div>
            <label className="block text-xs text-slate-300">Due date</label>
            <input
              name="dueDate"
              type="date"
              className="mt-1 w-full rounded-md border border-slate-700 bg-slate-950 px-3 py-2 text-sm outline-none focus:border-sky-500"
